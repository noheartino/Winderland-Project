...
export default function CartClass({ classItems, onRemove }) { // 接收 onRemove 作為 props
  ...
  return (
    <div>
      {classItems.map(item => (
        item.class_id !== null && ( // 确保 class_id 不是 null 才渲染
            ...
            <div className={css.cartClassContent}>
              ...
              <div className={css.cartClassContentText}>
                ...
                <div className={css.cartClassDel}>
                  <button onClick={() => onRemove(item.cart_item_id)}>✕</button> {/* 让这个按钮触发删除单个项目 */}
                </div>
              </div>
