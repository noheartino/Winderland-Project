...
export default function DashboardProfile() {
      ...
      const fetchAvatarUrl = async () => {
        try {
          const response = await fetch(
            `https://winderland.shop/api/dashboard/profile/avatar/${auth.userData.id}`
          );
          const data = await response.json();
          if (data.status === "success" && data.data.avatar_url) {
            setAvatarUrl(`https://winderland.shop${data.data.avatar_url}?t=${new Date().getTime()}`);
          } else {
            setAvatarUrl("/images/member/avatar/default-avatar.jpg");
          }
        } catch (error) {
  ...
  const handleAvatarChange = async (event) => {
    ...
    try {
      const response = await fetch(
        "https://winderland.shop/api/dashboard/profile/upload-avatar",
        {
          method: "POST",
          body: formData,
          credentials: "include",
      ...
      if (result.status === "success") {
        // 更新頭像 URL
        const newAvatarUrl = `https://winderland.shop${result.data.avatar_url}`;
