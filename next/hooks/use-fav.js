...
export function useFavorite(productId) {
    ...
    if (auth.userData && productId) {
      ...
      try {
        const response = await axios.get(
          `http://winderland.shop/api/favorites/products/check/${productId}`,
          {
  ...
  const toggleFavorite = async (productDetailId) => {
    ...
    try {
      if (isFav) {
        await axios.delete(
          `http://winderland.shop/api/favorites/products/${productId}`,
          {
            withCredentials: true,
          }
        );
        ...
        await axios.post(
          "http://winderland.shop/api/favorites/products",
